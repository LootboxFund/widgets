...
const ManageTokens = (props: ManageTokensProps) => {
	...
	return (
		<$SwapContainer>
			...
			<>
				<$Horizontal>
					<$Input value={searchString} onChange={(e) => setSearchString(e.target.value)} placeholder='Filter Custom Tokens...' style={{ fontWeight: 'lighter', border: `2px solid ${COLORS.warningBackground}30`, fontSize: '1.5rem', flex: 4 }}></$Input>
					<$Button
				...
				{
					customTokenList.filter(searchFilter).map(token => <RowToken key={token.symbol} token={token} copyable deleteable />)
				}
				{
					customTokenList.filter(searchFilter).length === 0 && <$NoTokensPrompt>No Custom Tokens Added Yet</$NoTokensPrompt>
				}
			</>
		</$SwapContainer>
	)
